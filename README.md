...
国内 NixOS 用户通常需要配置镜像源 以 解决各开发软件下载依赖库时的网络问题.

但是这些配置通常是繁琐的, 我审视了这个配置过程, 发现我们实际上只是在表达正交的五个维度的信息:

1. 我信任哪些镜像站
2. 当有多个选择时,我偏好选择哪个镜像站
3. 我要为哪些软件配置镜像站
4. 我怎么在 NixOS 中对不同软件配置镜像站
5. 各镜像站对于不同软件的准确下载地址是哪

而这五部分信息中, 我发现实际上需要我们定制的信息相当少:

1. 我信任哪些镜像站                         -- 有定制需求, 但仍然有很强的共性
2. 当有多个选择时,我偏好选择哪个镜像站      -- 这可能是最需要定制的
3. 我要为哪些软件配置镜像站                 -- 这实际上完全可以通过 NixOS 的模块系统自动获知
4. 我怎么在 NixOS 中对不同软件配置镜像站    -- 这是公共知识, 完全没必要让每个用户都自己研究和折腾一遍
5. 各镜像站对于不同软件的准确下载地址是哪   -- 这是公共知识, 完全没必要让每个用户都自己研究和折腾一遍

因此, 我设计了这个 NixOS 模块, 它允许你正交地表达各维度的偏好, 并且给出了合理的默认值, 因此你得以轻松地为所有软件配置镜像站.

> `mirrors-nix` 内置了 10+ 个常用 provider 预设 (tuna / ustc / aliyun / tencent / bfsu / sjtu / daocloud / hf-mirror / goproxy.cn / goproxy.io), 你只需声明偏好顺序, 模块自动解析出每个软件可用的镜像, 并添加对应的 NixOS 配置。
