# 默认场景断言 (mirrors.enable=true 全套默认行为)
#
# 职责边界:
#   本文件只描述 "应该断言什么" (声明式数据), 不负责如何运行断言.
#   checks/all/package.nix 读取这些 (label, expected, actual) 元组, 在 runCommand 中比对.
#
# 设计要点:
# - 每个断言是 { label; expected; actual; } 三元组, label 用于失败时定位.
# - expected 优先派生自 modules/providers.nix (SSOT), 避免硬编码 URL 导致双重维护.
...
#   lib:     nixpkgs.lib (仅用于 splitString / hasSuffix 等 list/字符串工具; 由 callPackage 从 scope 自动解析)
#   builtinPresets: 内置 provider 预设 (import modules/providers.nix), 用于派生 expected
#
# 易出错点:
...
let
  ...
  tunaNixUrl = builtinPresets.tuna.nix.url or null;
  tunaPypiUrl = builtinPresets.tuna.pypi.url or null;
  tunaCargoUrl = builtinPresets.tuna.cargo.url or null;
  tunaRustupUrl = builtinPresets.tuna.rustup.url or null;
  aliyunNpmUrl = builtinPresets.aliyun.npm.url or null;
  hfUrl = builtinPresets.hf-mirror.huggingface.url or null;
  firstGoproxyUrl = builtinPresets.goproxy-cn.goproxy.url or null;
...
in
# 派生式断言: expected 从 builtinPresets 读取, 修改 providers.nix 时自动跟随
[
  ...
  }
  {
    ...
    expected = "true";
    actual =
      if (builtins.length substituters) >= 4 then
        "true"
      else
        "false (got ${toString (builtins.length substituters)})";
  }
  {
    label = "nix.settings.trusted-public-keys is non-empty";
    expected = "true";
    actual =
      if (trusted-public-keys != [ ]) then
        "true"
      else
        "false";
  }
  ...
  # --- goproxy: 多镜像逗号拼接 + direct 兜底 ---
  {
    ...
    actual =
      ...
      if (builtins.length goproxyParts) >= 2 && lib.hasSuffix "direct" goproxy then
        "true"
      else
        "false (got '${goproxy}')";
  }
  {
    ...
    actual =
      if goproxyParts != [ ] then
        builtins.head goproxyParts
      else
        "<missing>";
  }
...
++ [
  {
    ...
    expected = "10";
    actual = toString (builtins.length (builtins.attrNames builtinPresets));
  }
  {
    label = "[non-derived] providers.nix tuna.nix.url 存在 (防字段误删)";
    expected = "true";
    actual =
      if builtinPresets ? "tuna" && builtinPresets.tuna ? "nix" && builtinPresets.tuna.nix ? "url" then
        "true"
      else
        "false";
  }
  {
    ...
    expected = "true";
    actual =
      if builtinPresets ? "aliyun" && builtinPresets.aliyun ? "npm" && builtinPresets.aliyun.npm ? "url" then
        "true"
      else
        "false";
  }
